...
void Panel::draw()
{
    if(_world != NULL)
    {
        ...
        glRotated(_camera->getPhi(),1,0,0);
        glRotated(_camera->getThe(),0,1,0);
...
void Controls::Mouse::_motion(int x, int y)
{
    ...
    if(_buttons[0])
    {
        _camera->setPhi(_camera->getPhi() + (float) 0.5f * diffy);
        _camera->setThe(_camera->getThe() + (float) 0.5f * diffx);
    }
    else if(_buttons[2])
    {
